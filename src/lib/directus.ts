...
function waslaProductAnswer(message: string, language: "ar" | "en") {
  ...
  if (/credit|wallet|balance|lead cost|رصيد|محفظ|خصم|كم عميل/.test(value)) {
    return ar
      ? "كل 20 رصيداً تساوي 1 ر.س وتكشف عميلاً واحداً. لا يُخصم شيء أثناء صياغة الطلب؛ الخصم يحدث فقط عند كشف سجل جديد، والسجل نفسه لا يُخصم مرتين. رصيد البداية متاح للحسابات الجديدة فوراً، وتوثيق الجوال اختياري."
      : "Every 20 credits equal 1 SAR and reveal one lead. Drafting a request costs nothing; credit is charged only when a new record is revealed, and the same record is never charged twice. Starter credit is available to new accounts immediately, and phone verification is optional.";
  }
